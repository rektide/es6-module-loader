...
(function (root, factory) {
  ...
  var g, gName, fetch, esprimaModule
  ...
  if (typeof XMLHttpRequest !== 'undefined') {
    ...
    throw 'Fetch not detected'
  }

  if (typeof define === 'function' && define.amd) {
    define('global', g)
    define('globalName', globalName)
    define('es6-module-loader',['exports','global','globalName','esprima'], function(exports,global,globalName,esprima){
      factory((global.Es6ModuleLoader= exports), global, globalName, esprima, fetch)
    })
    return
  } 

  try {
    esprimaModule = require('esprima')
  } catch (ex) {
    esprimaModule= global.esprima
  }
  if (!esprimaModule){
    throw 'Esprima not detected'
  }
  if (typeof exports !== 'undefined') {
    factory(exports, g, globalName, esprimaModule, esprimaModule, fetch);
  } else {
    factory((root.Es6ModuleLoader= {}), g, globalName, esprimaModule, fetch);
  }
}(this,function(exports,global,gName,esprima,fetch) {
  ...
  function Loader(options) {
    ...
    var Global = function () {}
    Global.prototype = options.global || global;
    ...
    Object.defineProperty(this._global, globalName, { value: this._global });
  ...
  var defaultSystemLoader = new Loader({
    ...
    resolve: function (name, options) {
      ...
      if (isUrl(name))
        return name;
      return this.baseURL + name + (name.substr(name.length - 3, 3) == '.js' ? '' : '.js');
    },
    fetch: fetch,
  ...
  if (typeof __filename !== undefined) {
    defaultSystemLoader.baseURL = ''
  } else if(typeof window !== "undefined") {
    var loc= window.location.href
    loc= loc.substring(0,loc.lastIndexOf('\/')+1)
    if (loc.substr(loc.length - 1, 1) != '/') loc += '/'
    defaultSystemLoader.baseURL = loc
  } else {
    throw 'No baseURL identified'
  }
